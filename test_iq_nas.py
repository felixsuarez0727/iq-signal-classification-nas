...
# ==========================
def classify_file(model, file_path, chunk_samples=512, batch_size=32):
    ...
    preds = model.predict(X_chunks, batch_size=batch_size, verbose=0)
...
# ==========================
def classify_folder(model_path, folder_path, chunk_samples=512, batch_size=32):
    ...
    for file in files:
        class_label, confidence, pred_labels = classify_file(model, file, chunk_samples, batch_size)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print("Usage: python test_iq_nas.py <model.keras> <test_folder> <chunk_samples> <batch_size>")
        sys.exit(1)
    ...
    classify_folder(model_path, folder_path, chunk_samples, batch_size)
